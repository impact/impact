...
We recommend that library developers make the most out of
the new features above and change the structure in which
they organize their library repositories.
\begin{enumerate}
...
to stay compatible with the \texttt{Modelica} library most user libraries chose the
...
\textbf{not} use the ``Modelica License Version 2.0'' for user libraries
...
other \mbox{non-GPL} library (even the Modelica Standard Library) uses
the GPL licensed library and is distributed that way. So what about the
...
version of a simulation model that uses different Modelica libraries.
...
Conclusion. User libraries should, if possible, avoid the ``Modelica
...
libraries that are BSD/MIT or MPL style.
...
Bitbucket(Mercurial and Subversion) repositories.
Pull requests to introduce such functionality would be welcome.
