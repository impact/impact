...
\hypersetup{%
  ...
  pdfkeywords = {Modelica, package management, GitHub, dependency resolution, golang},
  hidelinks,
...
connection is probably the biggest limiting factor.  So CPU
...
\item Until now, we advised the lib developers to keep the current
  ...
  no longer a need for a specific \texttt{release} branch. That is to say,
...
Up to May 2015 the listing pointed directly to the (tar)zip-ball URL of the
latest tag of a library. This worked fine if the library used the old
\texttt{release} branch model where the ``ready to install'' version
was placed.  Clicking on that coloured version link resulted in a
direct file download.

This has now been changed in such a way that if one clicks on the listed
``Last Release'' button one will get redirected to the ``Releases''
page of that project showing the last release. This has the advantage
that one does not immediately download the (tar)zip ball but gets to
...
Works'' which says that:

 ``\emph{\ldots This means especially that the
  (root-level) name of a Modelica package under this license must be
  changed if the package is modified \ldots}''.
...
license is the GNU General Public License. People might think this
...
\mbox{non-GPL} library (even the \emph{Modelica Standard Library}) uses the
