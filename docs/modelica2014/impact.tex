...
Search for librararies is done by executing:
...
\url{https://modelica.org/libraries} were moved to individual repositories
on GitHub. Third-party packages can be found under
\url{https://github.com/modelica-3rdparty}
and packages by the MA under \url{https://github.com/modelica}
...
\label{sec:semver}
One thing that is important when trying to build up a package manager that can
also handle version dependencies is that we need a proper system for the
versioning.

We decided to base our package manager on the system called
Semantic Versioning\cite{semver}.
Therefore we also advice all package developers to use semantic versioning
so that they are compatible with \impact\ and also give a proper meaning to
...
In addition also pre- and post-releases are taken care of in this system,
details can be found in the manual page\cite{semver}.
